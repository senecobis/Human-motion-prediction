...
class DCT_ATT_GCN(BaseModel):
    ...
    def create_model(self):
        ...
        self.normalize = nn.BatchNorm1d(num_features=self.hidden_feature, eps=1e-05, momentum=0.1, 
    ...
    def forward(self, batch: AMASSBatch):
        ...
        :return: a dictionary with keys {'seed', 'predictions'}.
        """
        ...
        src_value_tmp = src_value_tmp.reshape([batch_size, vn, self.n_dct_freq, -1]).transpose(2, 3).reshape([batch_size, vn, -1])

        idx = list(range(-self.kernel_size, 0, 1)) + [-1] * self.target_seq_len

        key_tmp = self.convK(src_key_tmp)
        key_tmp = self.normalize(key_tmp) # batch normalization

        query_tmp = self.convQ(src_query_tmp)
        query_tmp = self.normalize(query_tmp) # batch normalization

        # attention mechanism
        score_tmp = torch.matmul(query_tmp.transpose(1, 2), key_tmp) + 1e-15 # Q * K
